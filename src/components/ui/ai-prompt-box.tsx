      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              "relative group rounded-3xl border border-[#444444] bg-[#1F2023] p-2 shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300",
            ...
            onDrop={onDrop}
          >
            {/* Animated Glowing Bar Effect */}
            <div className="absolute -inset-[1.5px] -z-10 overflow-hidden rounded-[calc(1.5rem+1.5px)] opacity-0 group-focus-within:opacity-100 transition-opacity duration-500 blur-[2px]">
            ...
            <div className="absolute -inset-[1.5px] -z-10 overflow-hidden rounded-[calc(1.5rem+1.5px)] opacity-0 group-focus-within:opacity-40 transition-opacity duration-500 blur-[8px]">
              <div className="absolute inset-[-500%] animate-[spin_4s_linear_infinite] bg-[conic-gradient(from_0deg,transparent_0deg,transparent_300deg,#FF4D00_360deg)]" />
            </div>
            {children}
          </div>
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const recommendations = [
    "Search for Maruti Suzuki Swift brake pads...",
    "Find engine oil for Royal Enfield Classic 350...",
    "Looking for Hyundai i20 headlight assembly?",
    "Need a clutch plate for Tata Nexon?",
    "Find the best mechanics in Coimbatore...",
    "Check availability of Bosch spark plugs...",
  ];

  React.useEffect(() => {
    const interval = setInterval(() => {
      setPlaceholderIndex((prev) => (prev + 1) % recommendations.length);
    }, 3000);
    return () => clearInterval(interval);
  }, []);

  const currentPlaceholder = recommendations[placeholderIndex];
