...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center h-16 rounded-lg bg-card text-foreground border px-2 ${className}`}>
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 z-10 w-12 h-[3px] rounded-full bg-brand-orange shadow-[0_0_15px_rgba(255,77,0,0.8)] ${
          isReady ? 'transition-[left] duration-400 ease-in-out' : ''
        } ${limelightClassName}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-[-50%] top-[3px] w-[200%] h-16 [clip-path:polygon(10%_100%,35%_0,65%_0,90%_100%)] bg-gradient-to-b from-brand-orange/40 via-brand-orange/10 to-transparent pointer-events-none blur-[1px]" />
      </div>
    </nav>
  );
};
